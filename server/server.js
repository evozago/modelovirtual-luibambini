...
if (!apiKey) {
  ...
  console.log("Gemini API KEY FOUND (proxy will use this)")
}

if (!shopifyAdminToken || !shopifyShopName) {
  console.error("Warning: Shopify Admin API credentials not set in .env file. Product search will fail.");
} else {
  console.log("Shopify Admin credentials found.");
}
...
app.use('/shopify-admin-proxy', proxyLimiter);


// Secure proxy for Shopify Admin API
app.get('/shopify-admin-proxy/products', async (req, res) => {
    if (!shopifyAdminToken || !shopifyShopName) {
        return res.status(500).json({ message: 'Shopify Admin API not configured on the server.' });
    }

    const searchQuery = req.query.search || '';
    const shopifyApiUrl = `https://${shopifyShopName}.myshopify.com/admin/api/2024-04/graphql.json`;

    // A query que busca por sku, código de barras ou título.
    const graphqlQuery = `
    ...
    try {
        const response = await axios.post(
            shopifyApiUrl,
            {
                query: graphqlQuery,
                variables: {
                  // Constrói a query para buscar em múltiplos campos.
        ...
        if (response.data.errors) {
            ...
            return res.status(500).json({ message: 'Error from Shopify API.', details: response.data.errors });
        }

        // Mapeia a resposta para o formato esperado pelo frontend
        ...
        console.error('Error proxying to Shopify Admin API:', error.response ? error.response.data : error.message);
        res.status(502).json({ message: 'Failed to fetch data from Shopify.' });
    }
});
...
const server = app.listen(port, () => {
    ...
    if (apiKey) console.log(`HTTP proxy active for Gemini on /api-proxy/**`);
    if (apiKey) console.log(`WebSocket proxy active for Gemini on /api-proxy/**`);
    if (shopifyAdminToken) console.log(`HTTP proxy active for Shopify on /shopify-admin-proxy/**`);
});
